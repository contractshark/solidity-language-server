...
import { path2uri } from '../utilities';
  ...
  describe('LocalFileSystem', () => {
    ...
    beforeAll(async () => {
      temporaryDir = await new Promise<string>((resolve, reject) => {
        temp.mkdir('local-fs', (err: Error, dirPath: string) =>
      ...
      await fs.writeFile(
        path.join(somePackageDir, 'src', 'function.ts'),
        'foo',
      );
      ...
      await fs.writeFile(
        path.join(projectDir, '@types', 'diff', 'index.d.ts'),
        'baz',
      );
      ...
      await fs.symlink(
        somePackageDir,
        path.join(projectDir, 'node_modules', 'some_package'),
        'junction',
      );
