...
export function getCompletionsAtPosition(
  ...
  if (host.readFile) {
    ...
    const { triggeredByDot, wordEndCharacter } = isCompletionTriggeredByDot(
      lineText,
      position.character,
    );
    if (triggeredByDot) {
        ...
        case 'FunctionDeclaration':
          if (contractElement.name !== contractName) {
            completionItems.push(
              createFunctionEventCompletionItem(
                contractElement,
                'function',
                contractName,
              ),
...
function isCompletionTriggeredByDot(
  line: string,
  character: number,
): { triggeredByDot: boolean; wordEndCharacter: number } {
  let start = 0;
...
function getContextualCompletions(
  lineText: string,
  wordEndCharacter: number,
): CompletionItem[] {
  if (isCompletionTrigeredByVariableName('block', lineText, wordEndCharacter)) {
    return getBlockCompletions();
  } else if (
    isCompletionTrigeredByVariableName('msg', lineText, wordEndCharacter)
  ) {
    return getMsgCompletions();
  } else if (
    isCompletionTrigeredByVariableName('tx', lineText, wordEndCharacter)
  ) {
...
function isCompletionTrigeredByVariableName(
  variableName: string,
  lineText: string,
  wordEndCharacter: number,
): boolean {
...
function createFunctionEventCompletionItem(
  contractElement: any,
  type: string,
  contractName: string,
): CompletionItem {
...
function typeStringFromLiteral(literal: any) {
  ...
  if (typeof literalType.type !== 'undefined') {
    ...
    if (isMapping) {
      suffixType =
        '(' +
        typeStringFromLiteral(literalType.from) +
        ' => ' +
        typeStringFromLiteral(literalType.to) +
        ')';
    }
  }
...
function getBlockCompletions(): CompletionItem[] {
  return [
    ...
    {
      detail:
        '(bytes32): Hash of the given block - only works for 256 most recent blocks excluding current',
