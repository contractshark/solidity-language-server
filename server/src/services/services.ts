...
export function createLanguageService(
  ...
  const cancellationToken = new CancellationTokenObject(
    host.getCancellationToken && host.getCancellationToken(),
  );
  ...
  function synchronizeHostData(): void {
    ...
    const hasInvalidatedResolution: HasInvalidatedResolution =
      host.hasInvalidatedResolution || returnFalse;
    ...
    if (
      isProgramUptoDate(
        ...
        (path) => host.getScriptVersion(path),
        host.fileExists,
        hasInvalidatedResolution,
      )
    ) {
    ...
    function getOrCreateSourceFileByPath(
      ...
      const hostFileInformation = hostCache.getOrCreateEntryByPath(
        fileName,
        path,
      );
  ...
  function getCompilerDiagnostics(fileName: string): Diagnostic[] {
    ...
    return program
      .getCompilerDiagnostics(getValidSourceFile(fileName), cancellationToken)
      .slice();
  }

  function getSoliumDiagnostics(
    fileName: string,
    soliumRules: any,
  ): Diagnostic[] {
    synchronizeHostData();
  ...
  function getSolhintDiagnostics(
    fileName: string,
    soliumRules: any,
  ): Diagnostic[] {
    synchronizeHostData();
...
class CancellationTokenObject implements CancellationToken {
  ...
  public isCancellationRequested() {
    return (
      this.cancellationToken && this.cancellationToken.isCancellationRequested()
    );
  }
...
class HostCache {
  ...
  constructor(
    ...
    for (const fileName of rootFileNames) {
      this.createEntry(
        fileName,
        toPath(fileName, this.currentDirectory, getCanonicalFileName),
      );
    }
