...
export class ProjectManager {
   ...
   */
  public getChildConfigurations(
    uri: string,
  ): IterableIterator<ProjectConfiguration> {
  ...
  public ensureModuleStructure(): Observable<never> {
    if (!this.ensuredModuleStructure) {
      this.ensuredModuleStructure = this.updater
        ...
        .concat(
          Observable.defer(() =>
            observableFromIterable(this.inMemoryFs.uris()),
          ),
        )
  ...
  public ensureOwnFiles(): Observable<never> {
    if (!this.ensuredOwnFiles) {
      this.ensuredOwnFiles = this.updater
        ...
        .filter(
          (uri: string) =>
            (!uri.includes('/node_modules/') && isSolidityFile(uri)) ||
      ...
      this.ensureModuleStructure()
        ...
        .concat(
          Observable.defer(() =>
            maxDepth === 0
              ...
              : this.resolveReferencedFiles(uri),
          ),
        )
        // Prevent cycles
        ...
        .mergeMap((referencedUri) =>
          this.ensureReferencedFiles(referencedUri, maxDepth - 1, ignore)
            ...
            .catch((err: any) => {
              this.logger.error(
                `Error resolving file references for ${uri}:`,
                err,
              );
              return [];
            }),
...
export class ProjectConfiguration {
  ...
  private init(): void {
    ...
    this.expectedFilePaths = new Set(
      flatMap(configObject.include, (pattern) =>
        glob.sync(pattern, { cwd: this.rootFilePath }),
      ),
    );
