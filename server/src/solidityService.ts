...
export class SolidityService {
  ...
  constructor(
    ...
    protected options: SolidityServiceOptions = {},
  ) {
  ...
  public initialize(params: InitializeParams): Observable<Operation> {
    ...
    if (params.rootUri || params.rootPath) {
      ...
      this.globalProjectManager = this._createProjectManager({
        root: this.root,
   ...
   */
  private _createProjectManager(params: {
    root: string;
    rootUri: string;
  }): ProjectManager {
    // The remote (or local), asynchronous, file system to fetch files from
    const fileSystem = this.accessDisk
      ? new LocalFileSystem(params.rootUri)
  ...
  private getProjectManager(uri: string): ProjectManager {
    ...
    if (!projectManager) {
      projectManager = this._createProjectManager({
        root,
        rootUri: path2uri(root),
      });
   ...
   * settings.
   */
  public workspaceDidChangeConfiguration(
  ...
  public async textDocumentDidChange(
    ...
    for (const change of params.contentChanges) {
      if (change.range || change.rangeLength) {
        throw new Error(
          'incremental updates in textDocument/didChange not supported for file ' +
            uri,
        );
      }
      text = change.text;
  ...
  private _publishDiagnostics(uri: string): void {
    ...
    if (this.settings.solidity.solium.enabled) {
      const soliumDiagnostics = config
        .getService()
        .getSoliumDiagnostics(fileName, this.settings.solidity.solium.rules);
      diagnostics.push(...soliumDiagnostics);
    }
    if (this.settings.solidity.solhint.enabled) {
      const solhintDiagnostics = config
        .getService()
        .getSolhintDiagnostics(fileName, this.settings.solidity.solhint.rules);
  ...
  public textDocumentCompletion(
    ...
    // Ensure files needed to suggest completions are fetched
    return projectManager
      .ensureReferencedFiles(uri, undefined, undefined)
      .toArray()
      .mergeMap(() => {
        ...
        const completions = configuration
          .getService()
          .getCompletionsAtPosition(fileName, params.position);
