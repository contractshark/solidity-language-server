...
const defaultSettings: SoliditySettings = { maxNumberOfProblems: 1000 };
let globalSettings: SoliditySettings = defaultSettings;
...
connection.onDidChangeConfiguration(change => {
  if (hasConfigurationCapability) {
    ...
    globalSettings = <SoliditySettings>(
      (change.settings.languageServerSolidity || defaultSettings)
    );
  }
...
function getDocumentSettings(resource: string): Thenable<SoliditySettings> {
  if (!hasConfigurationCapability) {
